...
def render_setup_page():
    ...
    if not st.session_state.partners:
        ...
        partner_names = ", ".join(st.session_state.partners.keys())
        shared_note = " (includes Shared)" if st.session_state.get('has_shared_partner') else ""
        st.success(f"✅ **{len(st.session_state.partners)} Partners Configured:** {partner_names}{shared_note}")
